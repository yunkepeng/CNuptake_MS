#Data path from Yunke's local desktop:
#npp_path <- "~/data/NPP_Yunke/NPP_dataset.csv"
#nmin_path <- "~/data/Nmin_Finzi/Nmin_dataset.csv"
#leaf_traits_path <- "~/data/leaf_traits/combined_leaf_traits_updated.csv"
#woodcn_path <- "~/data/CN_wood/wood_cn.csv"
#NRE_path <- "~/data/NRE_various/NRE_dataset.csv"
#landcover_path <- "~/data/landcover/modis_landcover_halfdeg_2010_FILLED.nc"
#fland_path <- "~/data/fland/global.fland.nc"
#trendy_path <- "~/data/trendy/v8/"
#inputmap_path <- "~/data/nimpl_sofun_inputs/map/Final_ncfile/"

#Data path for further users (geco-users, and readers) to run:
#it was all already saved in this Github repository:
npp_path <- "data/NPP_Yunke/NPP_dataset.csv" # also saved in Geco-Archive: "/data/archive/NPP_Yunke/NPP_dataset.csv"
leaf_traits_path <- "data/leaf_traits/combined_leaf_traits_updated.csv" # also saved in Geco-Archive: "/data/archive/leaf_traits/combined_leaf_traits_updated.csv"
nmin_path <- "data/Nmin_Finzi/Nmin_dataset.csv" 
woodcn_path <- "data/CN_wood/wood_cn.csv"
NRE_path <- "data/NRE_various/NRE_dataset.csv"
landcover_path <- "data/landcover/modis_landcover_halfdeg_2010_FILLED.nc"
fland_path <- "data/fland/global.fland.nc"
trendy_path <- "data/trendy/v8/"
inputmap_path <- "data/nimpl_sofun_inputs/map/Final_ncfile/"



